...
exports.bookinstance_update_get = (req, res, next) => {
  ...
  (err, results) => {
    ...
    res.render("bookinstance_form", {
      ...
      selected_book: results.book_instance.book.id,
    });
  });
};
...
exports.bookinstance_update_post = [
  ...
  (req, res, next) => {
    ...
    const bookinstance = new BookInstance({
      ...
      imprint: req.body.imprint,
      status: req.body.status,
      due_back: rightDueDate(req.body.due_back),
      ...
      async.parallel(
        ...
        (err, results) => {
          ...
          res.render("bookinstance_form", {
            ...
            bookinstance,
            book_list: results.books,
            selected_book: book_instance.book.id,
            errors: errors.array(),
          });
        }
      );
...
const rightDueDate = (dueDate) => {
  let dueDateArr = dueDate.split('-');
  dueDateArr[2] = parseInt(dueDateArr[2]) + 1;
  return dueDateArr.join('-');
}